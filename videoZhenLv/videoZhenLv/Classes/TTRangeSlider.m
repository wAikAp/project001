...
const float HANDLE_DIAMETER = 25;

const float JINDU_X = 16.0f;
const float JINDU_Y = 22.0f;
...
@property (nonatomic, strong) CALayer *jinDuLine;
...
//    _selectedMaximum  = 90;
    ...
    //draw the slider line //线
    self.sliderLine = [CALayer layer];
    self.sliderLine.backgroundColor = [UIColor grayColor].CGColor;
    self.sliderLine.cornerRadius = 5.0f;
    [self.layer addSublayer:self.sliderLine];
    
    
    //进度条
    CALayer *jinDuLine = [CALayer layer];
    jinDuLine.backgroundColor = [UIColor blueColor].CGColor;
    self.jinDuLine = jinDuLine;
    jinDuLine.cornerRadius = 5;
    [self.layer addSublayer:jinDuLine];
    
    
    //draw the minimum slider handle //最小圆
    ...
    //draw the maximum slider handle //最大圆
...
- (void)layoutSubviews {
    ...
    self.sliderLine.frame = CGRectMake(lineLeftSide.x, lineLeftSide.y, lineRightSide.x-lineLeftSide.x,10);
...
//    self.minLabel.string = [NSString stringWithFormat:@"%.2f",self.selectedMinimum];
//    self.maxLabel.string = [NSString stringWithFormat:@"%.2f",self.selectedMaximum];
}
